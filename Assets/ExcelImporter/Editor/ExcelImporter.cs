...
public class ExcelImporter : AssetPostprocessor
{
	...
	{
		if (fieldInfo.FieldType.IsArray && cell.CellType == CellType.String)
		{
			...
			if (value.StartsWith("[") && value.EndsWith("]"))
			{
				...
				{
					return items.Select(s => s.Trim().Trim('"')).ToArray();
				}
				else if (fieldInfo.FieldType.GetElementType() == typeof(int))
				{
					return items.Select(s => int.Parse(s.Trim())).ToArray();
				}
			}
		}
